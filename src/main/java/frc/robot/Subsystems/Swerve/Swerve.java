...
public class Swerve extends SubsystemBase {
  ...
  public static class SwerveInputs {
    ...
    public SwerveInputs addtx(Double tx) {
      this.tx += tx;
      return this;
    }

    /** Update the input, returning itself */
    public SwerveInputs addty(Double tx) {
      this.ty += ty;
      return this;
    }

    /** Update the input, returning itself */
    public SwerveInputs addr(Double tx) {
      this.r += r;
      return this;
    }
  }
  ...
  public void periodic() {
    ...
    SmartDashboard.putNumber("swerve/currentSpeeds", Math.abs(swerveDrive.getRobotVelocity().vxMetersPerSecond)
        + Math.abs(swerveDrive.getRobotVelocity().vyMetersPerSecond));
   ...
   */
  public void setPrimaryInputsVoid(DoubleSupplier translationX, DoubleSupplier translationY,
      DoubleSupplier angularRotationX) {
    primaryInputs.tx = 1 * translationX.getAsDouble();
    primaryInputs.ty = 1 * translationY.getAsDouble();
    primaryInputs.r = angularRotationX.getAsDouble();
  ...
  /**
   * Add additional inputs for automatic actions like turning/aiming without
   * disrupting primary input.
   ...
   * @return
   */
  public Command addSecondaryInputs(DoubleSupplier translationX, DoubleSupplier translationY,
      DoubleSupplier angularRotationX) {
    return Commands.either(
        Commands.run(() -> {
  ...
  public Command addSecondaryInputsTrueFielcentric(DoubleSupplier translationX, DoubleSupplier translationY,
      DoubleSupplier angularRotationX) {
    return Commands.run(() -> {
      secondaryInputs.tx += 1 * translationX.getAsDouble();
      secondaryInputs.ty += 1 * translationY.getAsDouble();
  ...
  public Command turnToHeadingWithinTurretRange(Supplier<Rotation2d> bearing) {
    return Commands.run(() -> {
      ...
      if (Math.abs(error.getDegrees()) > 150.0) {
        secondaryInputs.r = output;
      } else {
        isOnTargetAngle = true;
  ...
  public Command setInitialPose(Pose2d targetPose) {
    return runOnce(() -> setInitialPoseVoid(targetPose));
  }
  ...
  public Command pidToPose(Supplier<Pose2d> targetPoseSupplier, double maxVelocityMPS, Distance tolerance) {
    return Commands.run(() -> {
      ...
      // double maxMagnitude = Math.hypot(xOutput, yOutput) *
      // maxVelocityMPS/maximumSpeed;
  ...
  // These are all super gross things needed for the PID command, please ignore,
  // and have to be set up
