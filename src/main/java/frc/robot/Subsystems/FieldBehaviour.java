...
public class FieldBehaviour {
  ...
  public FieldBehaviour() {
    ...
    if (DriverStation.isFMSAttached()){
      return; // Don't do debug things
    }

    new Trigger(DriverStation::isEnabled)
      .whileTrue(Commands.run(() -> {
        SmartDashboard.putBoolean("FieldBehaviour/retractHood",
          getRetractHood(field.getObject("TestPose").getPose()));
      })
    );
  ...
  public BoundingBox blueLowerTrenchHoodBox = new BoundingBox(centerX[0] - trenchHoodOffset, trenchY[0],
      centerX[0] + trenchHoodOffset, trenchY[1]);
  public BoundingBox blueUpperTrenchHoodBox = new BoundingBox(centerX[0] - trenchHoodOffset, trenchY[2],
      centerX[0] + trenchHoodOffset, trenchY[3]);
  public BoundingBox redUpperTrenchHoodBox = new BoundingBox(centerX[1] - trenchHoodOffset, trenchY[0],
